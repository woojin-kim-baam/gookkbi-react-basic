...
// - React에서는 addEventListener 함수를 사용하지 않고 인라인 방식 사용
...
export default function EventComponent() {
  ...
  const onKeyDownHandler = (event: KeyboardEvent<HTMLInputElement>) => {
    // const key = event.key;
    const { key } = event; // event 객체에서 key라는 속성을 추출하여 같은 이름의 변수 key에 할당.
    alert(`누른 키 : ${key}`);
  };
  ...
  // 이벤트 처리 콜백함수로 매개변수 event만 받는 형태가 아니라면 직접적으로 함수를 전달하여 사용할 수 없음
  // 이떄는 해당 이벤트 속성에 직접 익명 함수 표현식을 사용하여 전달
  const onEmployeeNameClickHandler = (employeeNumber: string) => {
    alert(`사원번호 : ${employeeNumber}`);
  };

  let count = 0; // 이와 같은 단순 변수는 화면에 바뀐 내용이 송출되지(렌더링) 않음
    ...
    <div>
      ...
      <div>
        {employees.map((employee, index) => (
          <div key={index}>
            <h2 onClick={() => onEmployeeNameClickHandler(employee.employeeNumber)}>
              {employee.name}
            </h2>
          </div>
        ))}
