...
export default function Axios() {
  ...
  // - post, patch, put : URL, requestBody, option[선택]

  const onBasicGet = () => {
    axios
      .get("http://127.0.0.1:8080/basic")
      // 각각의 http method는 then으로 response 객체를 받는 콜백 함수를 전달
      // then은 request에 대해서 성공인 경우 수행
      // then은 response의 http 상태 코드 100 ~ 300 처리
      .then((response) => {
  ...
  const onRequestBodyClickHandler = () => {
    // axios의 post, patch, put 요청에 두 번째 매개변수로 requestBody를 지정할 수 있음
    // 객체타입이어야 함
    //! post, patch, put 메서드의 두번째 매개변수는 반드시 requestBody임
    // ! request body 없이 option 만 지정하더라도 request body에는 빈 객체가 있어야 함
    axios
      .post("http://127.0.0.1:8080/request-data/request-body", { name, age })
      .then((response) => {
        const responseBody = response.data;
        setResponseBody(responseBody);
      })
